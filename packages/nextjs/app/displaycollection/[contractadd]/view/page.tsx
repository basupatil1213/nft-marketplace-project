...
export default function ViewImagesFromCollection({ params }: { params: { contractadd: string } }) {
  const { provider, account } = useWallet();
  const [nfts, setNFTs] = useState<NFT[]>([]);
  const { contractadd } = params;
  ...
  const fetchCollectionImages = async () => {
    ...
    try {
      ...
      const contract = contractStore.getCollectionContractFromAddress(contractadd);
      if (!contract) {
        console.error("Failed to get collection contract for address:", contractadd);
        setLoading(false);
        return;
      }
  ...
  const handleCreateAuction = (tokenId: string) => {
    router.push(`/auction?contractaddress=${contractadd}&tokenid=${tokenId}`);
  };
